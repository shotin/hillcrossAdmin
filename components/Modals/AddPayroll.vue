...
export default {
  ...
  mounted() {
    if (
      !this.form.breakdowns.earnings.length &&
      !this.form.breakdowns.deductions.length
    ) {
      this.addRow("all");
    }
  },
  ...
  methods: {
    ...
    deleteRow: function(type, index) {
      ...
      this.form.staffs = this.form.selectStaff.map((o) => o.id)
      this.$axios
        .post(`/payrolls`, this.form)
        .then((res) => {
          notify("Payroll created successfully", "success");
          this.$root.$emit("payroll_paginate", true);
          this.stopLoader();
          this.$bvModal.hide("add-payroll");
        })
        .catch((err) => {
